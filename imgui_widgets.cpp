...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ...
    // 根据id查询按键绑定
    std::map<ImGuiID, ImGuiKey>::iterator iter = g.HotKey_Setting.find(id);

    // 判断绑定按键是否被按下
    if (iter != g.HotKey_Setting.end() && IsKeyPressed(iter->second))
    {
        return true;
    }

    // 关闭设置
    if (g.HotKey_Open == true && (IsMouseClicked(0) || IsMouseClicked(2))) {
        g.HotKey_Open = false;
    }
